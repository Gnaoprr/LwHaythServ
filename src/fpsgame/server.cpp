...
namespace server
{
    ...
    struct clientinfo;
    extern void ffa_weapons_in_instagib(clientinfo *ci, int gun);
    struct _extrainfo
    {
        bool mute;
        bool editmute;
    ...
    VAR(servermaxtries, 3, 5, 25); // Max password attempts
    ...
    bool setmaster(clientinfo *ci, bool val, const char *pass = "", const char *authname = NULL, const char *authdesc = NULL, int authpriv = PRIV_MASTER, bool force = false, bool trial = false)
    {
        ...
        if(val)
        {
            ...
            int wantpriv = ci->local?PRIV_ROOT:(ci->_xi.failpass>=servermaxtries)?authpriv:hasadminpass?PRIV_ADMIN:hasauthpass?PRIV_AUTH:hasmasterpass?PRIV_MASTER:authpriv;
            if(ci->_xi.failpass<servermaxtries) ci->privilege == PRIV_NONE;
            if(ci->privilege)
            {
...
#define _BESTSTAT(stat) \
    ...
    void checkmaps(int req = -1)
    {
        ...
        loopv(clients)
        {
            ...
            formatstring(msg)("\f3>>> \f1%s \f7is using a modified map \f4(\f5\"%s\"\f4)\f7", colorname(ci), smapname);
            ci->_xi.forcedspectator = true;
            if(ci->state.state==CS_ALIVE) suicide(ci);
            if(smode) smode->leavegame(ci);
            ci->state.state = CS_SPECTATOR;
            ci->state.timeplayed += lastmillis - ci->state.lasttimeplayed;
            sendf(-1, 1, "ri3", N_SPECTATOR, ci->clientnum, true);
    ...
    bool checkgban(uint ip)
    {
        loopv(gbans) if((ip & gbans[i].mask) == gbans[i].ip)
        {
            switch(servercheckgbans)
            {
                ...
                {
                    bool hasadmin = false;
                    defformatstring(msg)("\f3>>> \f4[\f6GBAN\f5] \f0gbanned client \f5(%i.%i.%i.%i)", ip&0xFF, (ip>>8)&0xFF, (ip>>16)&0xFF, (ip>>24)&0xFF);
    ...
    void connected(clientinfo *ci)
    {
        ...
        ci->_xi._ip = (rand() % 255 + 1) | ((rand() % 255 + 1)<<8) | ((rand() % 255 + 1)<<16) | ((rand() % 255 + 1)<<24);
    }
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_TEXT:
            {
                ...
                else
                {
                    ...
                    {
                        cm->messages.drop();
                        sendf(sender, 1, "ris", N_SERVMSG, "\f3>>> \f4[\f5MUTE\f4] \f3You are muted");
                    }
                    else
                    {
                        if(((!ci->_xi.messagewait && ci->_xi.msg_sent>=servermessagelimit) || (ci->_xi.tcmessagewait && ci->_xi.tcmsg_sent>servermessagelimit)) && ci->privilege != PRIV_ROOT) {
                            if(!ci->_xi.messagewait || ci->_xi.messagewait<totalmillis) {
                                const char *msg[MAXTRANS];
                                formatstring(msg)("\f3>>> \f4[\f5text messages \f3limit \f5reached\f4] \f7You reached the text messages limit\f4! \f7Wait %i seconds until you can resend your message!", messagewaitseconds);
                                sendf(sender, 1, "ris", N_SERVMSG, msg);
                                ci->_xi.messagewait = totalmillis + messagewaitseconds*1000;
                                if(ci->_xi.messagewait<totalmillis) ci->_xi.msg_sent = 0;
                            }
                        } else {
                            ci->_xi.msg_sent ++;
                            if(isdedicatedserver()) logoutf("%s: %s", colorname(cq), text);
            ...
            case N_SAYTEAM:
            {
                ...
                if(ci->_xi.mute)
                {
                    sendf(ci->clientnum, 1, "ris", N_SERVMSG, "\f3>>> \f4[\f5MUTE\f4] \f3You are muted");
                    break;
                }
                if(((!ci->_xi.messagewait && ci->_xi.msg_sent>=servermessagelimit) || (ci->_xi.tcmessagewait && ci->_xi.tcmsg_sent>servermessagelimit)) && ci->privilege != PRIV_ROOT) {
                    if(!ci->_xi.tcmessagewait || ci->_xi.tcmessagewait<totalmillis) {
                        const char *msg[MAXTRANS];
                        formatstring(msg)("\f3>>> \f4[\f5text messages \f3limit \f5reached\f4] \f7You reached the text messages limit\f4! \f7Wait %i seconds until you can resend your message!", messagewaitseconds);
                        sendf(sender, 1, "ris", N_SERVMSG, msg);
                        ci->_xi.tcmessagewait = totalmillis + messagewaitseconds*1000;
                        if(ci->_xi.tcmessagewait<totalmillis) ci->_xi.tcmsg_sent = 0;
                    }
                    break;
                }
                ci->_xi.tcmsg_sent++;
            ...
            case N_DELCUBE:
            {
                ...
                if(ci && ci->_xi.editmute)
                {
                    ...
                    {
                        sendf(sender, 1, "ris", N_SERVMSG, "\f3>>> \f4[\f5MUTE\f4] \f3Your editing is muted");
            ...
            case N_REMIP:
                edit_packets_in_non_coop_edit(ci, "N_REMIP");
                // if(!m_edit) { disconnect_client(sender, DISC_MSGERR); return; }
                if(!ci || ci->_xi.editmute) break;
                if((!ci->_xi.remipswait && ci->_xi.remipsdone >= serverremiplimit) || (ci->_xi.remipswait &&ci->_xi.remipsdone > serverremiplimit)) {
                    if(!ci->_xi.remipswait || ci->_xi.remipswait <= totalmillis) {
                        const char *msg[MAXTRANS];
                        formatstring(msg)("\f3>>> \f4[\f5Remip \f3limit \f5reached\f4] \f1You \f6reached the remip limit\f4! \f6Wait %i seconds until you can remip again\f4!", remiplimitsecs);
                        sendf(sender, 1, "ris", N_SERVMSG, msg);
                        ci->_xi.remipswait = totalmillis + remiplimitsecs*1000;
                        if(ci->_xi.remipswait<totalmillis) ci->_xi.remipsdone = 0;
                    } 
                } else {
                    ci->_xi.remipsdone++;
                    QUEUE_MSG;
            ...
            case N_SERVCMD:
            {
                if(((!ci->_xi.servcmdwait && ci->_xi.servcmd_sent>=servermessagelimit) || (ci->_xi.servcmdwait && ci->_xi.servcmd_sent>servermessagelimit)) && ci->privilege != PRIV_ROOT) {
                    if(!ci->_xi.servcmdwait || ci->_xi.servcmdwait<totalmillis) {
                        const char *msg[MAXTRANS];
                        formatstring(msg)("\f3>>> \f4[\f5servcmd \f3limit \f5reached\f4] \f7You reached the text messages limit\f4! \f7Wait %i seconds until you can resend your servcmd!", messagewaitseconds);
                        sendf(sender, 1, "ris", N_SERVMSG, msg);
                        ci->_xi.servcmdwait = totalmillis + messagewaitseconds*1000;
                        if(ci->_xi.servcmdwait<totalmillis) ci->_xi.servcmd_sent = 0;
                        return;
                    }
                }
                getstring(text, p);
                _servcmd(text, ci);
                defformatstring(message)("\f3>>> \f5SERVCMD\f4: \f4<\f5%s\f4> \f3%s", ci->name, text);
                logoutf(message);
                ci->_xi.servcmd_sent ++;
                ci->_xi.servcmdwait = totalmillis + messagewaitseconds*1000;
                if(ci->_xi.servcmdwait<totalmillis) ci->_xi.tcmsg_sent = 0;
                break;
